 ...
 * // 想通过用C++实现，本节课Code05_KdtLazyTag4文件就是C++的实现
 * // 两个版本的逻辑完全一样，C++版本可以通过所有测试
 */
public class Code05_KdtLazyTag3 {
    ...
    public static void maintain(int i) {
        ...
        for (int d = 0; d < k; d++) {
            minv[i][d] = Math.min(pos[i][d], Math.min(minv[ls[i]][d],
                minv[rs[i]][d]));
            maxv[i][d] = Math.max(pos[i][d], Math.max(maxv[ls[i]][d],
                maxv[rs[i]][d]));
        }
    }
