...
public class Code05_Jump1 {
    ...
    public static PriorityQueue<int[]> heap = new PriorityQueue<>((a, b) ->
        a[0] - b[0]);
    ...
    public static void jumpToRectangle(int jl, int jr, int jd, int ju,
                                       int jdist, int i) {
        if (i == 0) {
            return;
        }
            ...
            if (!vis[i]) {
                ...
                if (i <= n) {
                    for (int e = headj[i]; e > 0; e = nextj[e]) {
                        int j = toj[e];
                        jumpToRectangle(jump[j][1], jump[j][2],
                            jump[j][3], jump[j][4], d + jump[j][0], root);
                    }
                } else {
