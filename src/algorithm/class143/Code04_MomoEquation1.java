...
public class Code04_MomoEquation1 {
    ...
    private static void dijkstra() {
        ...
        while (!heap.isEmpty()) {
            ...
            for (int e = head[u], v; e != 0; e = next[e]) {
                v = to[e];
                if (!visited[v] && distance[v] > weight[e] + w) {
                    distance[v] = weight[e] + w;
    ...
    private static void addEdge(int u, int v, long w) {
        next[cnt] = head[u];
        to[cnt] = v;
        weight[cnt] = w;
        head[u] = cnt++;
