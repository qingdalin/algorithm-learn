...
public class Code06_WiggleSortII {
    ...
    private static void circle(int[] arr, int l, int r, int i) {
        for (int j = 1, cur, next, curv, nextv, init; j <= i; j++) {
    ...
    private static int to(int i, int l, int r) {
        if (i <= (l + r) >> 1) {
            return i + (i - l + 1);
        } else {
            return i - (r - i + 1);
    ...
    private static void build(int n) {
        size = 0;
        for (int s = 1, p = 2; s <= n; s *= 3, p = s * 3 - 1) {
            start[++size] = s;
            split[size] = p;
        }
    }
    ...
    // 时间复杂度O(n)，额外空间复杂度O(1)
    private static int randomizedSelect(int[] arr, int n, int i) {
        int ans = 0;
