...
public class Code07_MaximumOfShortestPath1 {
    ...
    public static int n, m, q, cntn, k;
    ...
    public static void sortByLen(int[] nums, int l, int r) {
        ...
        int pivot = nums[(l + r) >> 1];
        while (i <= j) {
            while (cycleLen[nums[i]] < cycleLen[pivot]) {
                i++;
            }
            while (cycleLen[nums[j]] > cycleLen[pivot]) {
                j--;
            }
            if (i <= j) {
                int tmp = nums[i];
        ...
        for (int i = 1; i <= siz * 2; i++) {
            ...
            if (l <= r) {
                diameter = Math.max(diameter,
                    val[que[l]] - pre[que[l]] + val[i] + pre[i]);
            }
