...
public class Code05_Bomb1 {
    ...
    public static void rangToX(int jobl, int jobr, int jobx, int l, int r, int i) {
        if (jobl <= l && r <= jobr) {
            addEdge(i, jobx);
            saveEdge(i, jobx);
        } else {
            int mid = (l + r) >> 1;
            if (jobl <= mid) {
                rangToX(jobl, jobr, jobx, l, mid, ls[i]);
            }
            if (jobr > mid) {
                rangToX(jobl, jobr, jobx, mid + 1, r, rs[i]);
            }
        }
    }
