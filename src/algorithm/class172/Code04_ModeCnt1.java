...
public class Code04_ModeCnt1 {
    ...
    public static void prepare() {
        ...
        bnum = (n + blen - 1) / blen;
        for (int i = 1; i <= n; i++) {
            // 错误写法 bi[i] = (i - 1) * blen + 1;
            bi[i] = (i - 1) / blen + 1;
        }
        for (int i = 1; i <= bnum; i++) {
            bl[i] = (i - 1) * blen + 1;
            br[i] = Math.min(blen * i, n);
        }
        for (int i = 1; i <= n; i++) {
            sortList[i][0] = arr[i];
            sortList[i][1] = i;
    ...
    public static int query(int l, int r) {
        ...
        if (bi[l] == bi[r]) {
            ...
            for (int i = l, idx; i <= br[bi[l]]; i++) {
                idx = listIdx[i];
                // 错误写法 while (idx + ans <= n && sortList[idx + ans][0] == arr[i] && idx + ans <= r) {
            ...
            for (int i = bl[bi[r]], idx; i <= r; i++) {
                idx = listIdx[i];
                // 错误写法 while (idx - ans >= 1 && sortList[idx - ans][0] == arr[i] && idx - ans >= l) {
