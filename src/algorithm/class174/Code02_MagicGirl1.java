...
public class Code02_MagicGirl1 {
    ...
    public static void radix(int[] idx, int[] val, int size) {
        Arrays.fill(cntv, 0);
        for (int i = 1; i <= size; i++) {
            cntv[val[idx[i]] & OFFSET]++;
        }
        for (int i = 1; i < MAXB; i++) {
            cntv[i] += cntv[i - 1];
        }
        for (int i = size; i >= 1; i--) {
            // 错误写法 help[cntv[val[idx[i] & OFFSET]]--] = idx[i];
            help[cntv[val[idx[i]] & OFFSET]--] = idx[i];
        }
        for (int i = 1; i <= size; i++) {
            idx[i] = help[i];
        }
        ...
        for (int i = size; i >= 1; i--) {
            // 错误写法 help[cntv[val[idx[i] >> POW]]--] = idx[i];
            help[cntv[val[idx[i]]  >> POW]--] = idx[i];
        }
        for (int i = 1; i <= size; i++) {
            idx[i] = help[i];
        }
