...
public class Code04_IndexWithSegment1 {
    ...
    public static int cntadd, cntminus;

    public static int kth(int num) {
        int left = 0, right = s, mid;
        while (left <= right) {
            mid = (left + right) / 2;
            if (sorted[mid] == num) {
                return mid;
    ...
    public static void add(int i, int cnt) {
        for (int j = i; j <= n; j += lowbit(j)) {
            // 错误写法 root[j] = innerAdd(arr[i], 1, 1, s, root[j]);
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1, op, x, y, z; i <= m; i++) {
            in.nextToken(); ques[i][0] = (int) in.nval;
            in.nextToken(); ques[i][1] = (int) in.nval;
            in.nextToken(); ques[i][2] = (int) in.nval;
            if (ques[i][0] != 3) {
                in.nextToken(); ques[i][3] = (int) in.nval;
            }
        }
        prepare();
        for (int i = 1, op, x, y, z; i <= m; i++) {
