...
public class Code08_Taiko1 {
    ...
    public static int u, e, stacksize;
    ...
    public static void euler1(int u, int e) {
        while (cur[u] < k) {
            int ne = cur[u]++;
            euler1((u * k + ne) % m, ne);
        }
    ...
    public static void euler2(int node, int edge) {
        stacksize = 0;
        push(node, edge);
        while (stacksize > 0) {
            pop();
    ...
    public static String crackSafe(int len, int num) {
        prepare(len, num);
        euler1(0, -1);
//            euler2(0, -1);
