...
public class Code01_Fenwick_Tree_Optimization_java {
    ...
    public static void rangToX(int p, int x, int w) {
        while (p > 0) {
            if (outTree[p] > 0) {
    ...
    public static void rangToRange(int p1, int p2, int w) {
        int x = ++cntt;
        int y = ++cntt;
        rangToX(p1, x, 0);
        xToRange(y, p2, 0);
    ...
    public static void rangToX_2(int p, int x, int w) {
...
//                    System.out.println("优化建图的耗时：" + (end - start));
//                    start = System.currentTimeMillis();
                    int ans2 = dijkstra2(x, y);
//                    end = System.currentTimeMillis();
//                    System.out.println("暴力建图的耗时：" + (end - start));
                    if (ans1 != ans2) {
