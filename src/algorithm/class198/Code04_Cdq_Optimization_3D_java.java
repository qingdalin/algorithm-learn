...
public class Code04_Cdq_Optimization_3D_java {
    ...
    public static void rangToX(int i, int x, int w) {
        while (i > 0) {
            if (tree[i] > 0) {
                addEdge(tree[i], x, w);
            }
    ...
    public static void buildGraph() {
        ...
        for (int l = 1, r = 1; l <= n; l = ++r) {
            int a = arr[l][1], b = arr[l][2], c = arr[l][3];
            while (r + 1 <= n && a == arr[r + 1][1]
                && b == arr[r + 1][2] && c == arr[r + 1][3]) {
                r++;
            }
            int x = ++cntt;
            for (int i = l; i <= r; i++) {
    ...
    public static void clear() {
        ...
        cntg = cntt = cntp = cnta = cnt_2 = maxc = 0;
    }
