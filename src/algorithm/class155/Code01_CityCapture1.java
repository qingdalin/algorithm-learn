...
public class Code01_CityCapture1 {
    ...
    public static void compute() {
        ...
        for (int i = n; i >= 1; i--) {
            // top[i]有骑士，并且攻击小于城市防御，则弹出
            while (top[i] != 0 && attack[top[i]] < defend[i]) {
                die[top[i]] = i; // top[i]的骑士死在了i号城市
                sacrifice[i]++; // 牺牲骑士数++
                top[i] = pop(top[i]);
            }
            if (top[i] != 0) {
...
//            defend[i + 1] = Long.parseLong(arr[i]);
//        }
//        for (int i = 2; i <= n; i++) {
//            String[] line = bf.readLine().split(" ");
//            belong[i] = Integer.parseInt(line[0]);
//            type[i] = Integer.parseInt(line[1]);
//            gain[i] = Long.parseLong(line[2]);
//        }
//        for (int i = 1; i <= m; i++) {
//            String[] line = bf.readLine().split(" ");
//            attack[i] = Long.parseLong(line[0]);
...
//        }
//        for (int i = 1; i <= m; i++) {
//            out.println(deep[first[i]] - deep[die[i]]);
//        }
//        out.flush();
//        out.close();
//        bf.close();
