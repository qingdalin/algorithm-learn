...
public class Code06_XorOperation1 {
    ...
    public static int insert(int num, int i) {
        ...
        tree[rt][1] = tree[i][1];
        pass[rt] = pass[i] + 1;
        for (int b = BIT, cur, path, pre = rt; b >= 0; b--, pre = cur) {
            path = (num >> b) & 1;
            i = tree[i][path];
            cur = ++cnt;
            tree[cur][0] = tree[i][0];
    ...
    public static int maxKth(int xl, int xr, int yl, int yr, int k) {
        ...
        for (int b = BIT, best, path, sum; b >= 0; b--) {
            sum = 0;
            // 统计x[xl...xr]范围上
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0, xl, xr, yl, yr, k; i < p; i++) {
            in.nextToken(); xl = (int) in.nval;
            in.nextToken(); xr = (int) in.nval;
            in.nextToken(); yl = (int) in.nval;
            in.nextToken(); yr = (int) in.nval;
