...
public class Code05_FireFighting2 {
    ...
    private static int compute() {
        ...
        for (int l = end, r = end; l != 0; l = last[l]) {
            ...
            ans = Math.min(ans, Math.max(Math.max(suml, diameter - sumr), maxDist[queue[h]]));
            if (queue[h] == l) {
                h++;
            }
            suml += pred[l];
    ...
    private static int maxDistanceExceptDiameter(int u, int f, int c) {
        int ans = c;
    ...
    public static int[][] ufew = new int[MAXN][4];
    public static int u, f, e, w;
    public static int stackSize;

    public static void push(int u, int f, int e, int w) {
        ufew[stackSize][0] = u;
        ufew[stackSize][1] = f;
        ufew[stackSize][2] = e;
        ufew[stackSize][3] = w;
