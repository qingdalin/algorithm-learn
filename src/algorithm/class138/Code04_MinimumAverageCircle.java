...
public class Code04_MinimumAverageCircle {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= m; i++) {
            ...
            in.nextToken(); double w = in.nval;
            addEdge(u, v, w);
        }
        double l = -MAXE, r = MAXE, x, ans = 0;
        while (l < r && r - l >= sml) {
            x = (r + l) / 2;
            // 校验成功说明出现负环了，减的x比平均值要大，去左侧二分
            // ，每个值都变的更小了，不记录答案，向左二分
            // 要求返回的是更小的答案值，所以不记录
            if (check(x)) {
                r = x - sml;
            } else {
    ...
    // u=0设置一个无穷远的点，到某个点的权值是0
    private static boolean dfs(int u, double x) {
        if (u == 0) {
            for (int i = 1; i <= n; i++) {
                if (dfs(i, x)) {
                    return true;
