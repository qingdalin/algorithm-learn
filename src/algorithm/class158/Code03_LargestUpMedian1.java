...
public class Code03_LargestUpMedian1 {
    ...
    public static int update(int jobi, int l, int r, int i) {
        ...
        if (l == r) {
            ...
            if (jobi <= mid) {
                ...
                right[rt] = update(jobi, mid + 1, r, right[i]);
            }
            up(rt);
        }
        return rt;
    ...
    public static void main(String[] args) throws IOException {
        ...
        int lastAns = 0;
        for (int i = 1; i <= m; i++) {
            in.nextToken(); ques[0] = ((int) in.nval + lastAns) % n + 1;
            in.nextToken(); ques[1] = ((int) in.nval + lastAns) % n + 1;
            in.nextToken(); ques[2] = ((int) in.nval + lastAns) % n + 1;
            in.nextToken(); ques[3] = ((int) in.nval + lastAns) % n + 1;
            Arrays.sort(ques);
